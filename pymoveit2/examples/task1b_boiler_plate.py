    ...
    def process_image(self):
        ...
        if ids is not None:
            for i in range(min(len(angle_aruco_list), 3)):  # Process up to 3 markers
                ...
                if angle<-0.8:#The boxes on the left_On the same level
                    quaternion = tf_transformations.quaternion_from_euler(1.57, c_pitch,1.57-corrected_angle)
                elif angle>0.8:#The boxes on the right_On the same level
                    quaternion = tf_transformations.quaternion_from_euler(1.57, -c_pitch,1.57-corrected_angle)
                elif angle<0.5 and angle>-0.5:#The boxes in the front_On the same level
